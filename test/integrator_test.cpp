...
class IntegratorTest : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    all_parts = malloc(particle_count * sizeof(Particle));
...
TEST_F(IntegratorTest, index_Test) {
  EXPECT_EQ(particle_count + 3, pair_index(1, 3));
  EXPECT_EQ(particle_count + 3, pair_index(3, 1));
}

TEST_F(IntegratorTest, create_Test) {
  EXPECT_FLOAT_EQ(5.0f, calc->square_sum_radii[pair_index(0, 1)]);
  EXPECT_FLOAT_EQ(2.0f, calc->pair_restitution[pair_index(0, 1)]);
  EXPECT_FLOAT_EQ(1.5f, calc->pair_inv_mass[pair_index(0, 1)]);
}
